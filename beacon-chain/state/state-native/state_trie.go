...
const (
	...
	experimentalStateEip7805SharedFieldRefCount   = 10
)
...
// InitializeFromProtoEip7805 the beacon state from a protobuf representation.
func InitializeFromProtoEip7805(st *ethpb.BeaconStateElectra) (state.BeaconState, error) {
	return InitializeFromProtoUnsafeEip7805(proto.Clone(st).(*ethpb.BeaconStateElectra))
}
...
// InitializeFromProtoUnsafeEip7805 directly uses the beacon state protobuf fields
// and sets them as fields of the BeaconState type.
func InitializeFromProtoUnsafeEip7805(st *ethpb.BeaconStateElectra) (state.BeaconState, error) {
	if st == nil {
		return nil, errors.New("received nil state")
	}
	...
	fieldCount := params.BeaconConfig().BeaconStateEip7805FieldCount
	b := &BeaconState{
	...
	if features.Get().EnableExperimentalState {
		...
		b.validatorsMultiValue = NewMultiValueValidators(st.Validators)
		b.inactivityScoresMultiValue = NewMultiValueInactivityScores(st.InactivityScores)
		b.sharedFieldReferences = make(map[types.FieldIndex]*stateutil.Reference, experimentalStateEip7805SharedFieldRefCount)
	} else {
		bRoots := make([][32]byte, fieldparams.BlockRootsLength)
		for i, r := range st.BlockRoots {
			bRoots[i] = bytesutil.ToBytes32(r)
		...
		b.sharedFieldReferences = make(map[types.FieldIndex]*stateutil.Reference, eip7805SharedFieldRefCount)
	}

	for _, f := range eip7805Fields {
		b.dirtyFields[f] = true
		b.rebuildTrie[f] = true
		b.dirtyIndices[f] = []uint64{}
		trie, err := fieldtrie.NewFieldTrie(f, types.BasicArray, nil, 0)
...
func (b *BeaconState) Copy() state.BeaconState {
	...
	if features.Get().EnableExperimentalState {
		...
		case version.Eip7805:
			dst.sharedFieldReferences = make(map[types.FieldIndex]*stateutil.Reference, experimentalStateEip7805SharedFieldRefCount)
		}
	} else {
		switch b.version {
		case version.Phase0:
		...
		case version.Eip7805:
			dst.sharedFieldReferences = make(map[types.FieldIndex]*stateutil.Reference, eip7805SharedFieldRefCount)
		}
	}
...
func (b *BeaconState) initializeMerkleLayers(ctx context.Context) error {
	...
	case version.Eip7805:
		b.dirtyFields = make(map[types.FieldIndex]bool, params.BeaconConfig().BeaconStateEip7805FieldCount)
	default:
