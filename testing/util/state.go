...
// NewBeaconStateEip7805 creates a beacon state with minimum marshalable fields.
func NewBeaconStateEip7805(options ...func(state *ethpb.BeaconStateElectra) error) (state.BeaconState, error) {
	pubkeys := make([][]byte, 512)
	for i := range pubkeys {
		pubkeys[i] = make([]byte, 48)
	}
	...
	var st, err = state_native.InitializeFromProtoUnsafeEip7805(seed)
	if err != nil {
		return nil, err
	}
