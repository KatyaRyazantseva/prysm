...
func TestProposeBeaconBlock_Eip7805(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	jsonRestHandler := mock.NewMockJsonRestHandler(ctrl)

	var blockContents structs.SignedBeaconBlockContentsEip7805
	err := json.Unmarshal([]byte(rpctesting.Eip7805BlockContents), &blockContents)
	require.NoError(t, err)
	genericSignedBlock, err := blockContents.ToGeneric()
	require.NoError(t, err)

	eip7805Bytes, err := json.Marshal(blockContents)
	require.NoError(t, err)
	...
	expectedBlockRoot, err := genericSignedBlock.GetEip7805().Block.HashTreeRoot()
	require.NoError(t, err)
